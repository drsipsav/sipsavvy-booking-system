...
function selectPackage(name, guests, price, description) {
...
function setTravelFee(amount) {
  saveBookingState({ travelFee: amount });
  syncAllPrices();
}
...
function syncAllPrices() {
  ...
  if (timeFrameEl) {
    var pkgBaseHrs = { essential: 3, luxe: 4, signature: 5 };
    var baseHrs = pkgBaseHrs[window.currentPkgKey] || 3;
    var tfTotal = baseHrs + (extra / 125);
    timeFrameEl.innerHTML = 'Your selected package runs approximately <strong>' + tfTotal + ' hour' + (tfTotal !== 1 ? 's' : '') + '</strong>. The start time you pick is when service begins. Please ensure your venue is ready.';
  }
  }
