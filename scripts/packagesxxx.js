// ---------------------------------------------------------
// Imports (global key map)
// ---------------------------------------------------------
import { SS_KEYS } from "./ssKeys.js";
...
// Resets every package card EXCEPT the one passed in.
// Called both at the START and END of unifiedSelectPackage
// so that no load* or update* function can re-show a
...
// ---------------------------------------------------------
// updateTotalsDisplay — reads all totals from localStorage
// and refreshes every display element on the current page.
...
function updateTotalsDisplay() {
  ...
  const travelFee  = Number(localStorage.getItem("ss_travelFee")  || 0);
  ...
  // Checkout bar totals (if present)
  const checkoutTotal = document.getElementById("checkoutTotal");
  if (checkoutTotal) checkoutTotal.textContent = `$${grand.toFixed(2)}`;
}

// ---------------------------------------------------------
// UNIFIED TRAVEL FEE ENGINE
// ---------------------------------------------------------
...
function updateContainer4Summary() {
  ...
  const pkgNameEl    = document.querySelector('[data-summary="packageName"]');
  const pkgPriceEl   = document.querySelector('[data-summary="packagePrice"]');
  const pkgDescEl    = document.querySelector('[data-summary="packageDesc"]');
  const pkgGuestsEl  = document.querySelector('[data-summary="packageGuests"]');
  const pkgEventEl   = document.querySelector('[data-summary="packageEvent"]');
  const addonsTotalEl = document.querySelector('[data-summary="addons-total"]');
  const travelFeeEl  = document.querySelector('[data-summary="travel-fee"]');
  const grandTotalEl = document.querySelector('[data-summary="grand-total"]');

  if (pkgNameEl)   pkgNameEl.textContent   = name;
  if (pkgPriceEl)  pkgPriceEl.textContent  = `$${price.toFixed(2)}`;
  if (pkgDescEl)   pkgDescEl.textContent   = desc;
  if (pkgGuestsEl) pkgGuestsEl.textContent = guests;
  if (pkgEventEl)  pkgEventEl.textContent  = event;
  ...
  if (travelFeeEl) travelFeeEl.textContent = `$${travelFee.toFixed(2)}`;
...
/* ============================
   PACKAGE SELECTION
   KEY FIX: resetAllPackageCards() is called BOTH at the
   start (before load* functions run) AND at the end (after
   load* functions run), so no intermediate re-render can
   leave another card in a "Selected" state.
   All load* / update* calls are wrapped in try/catch so
   a missing function on this page never crashes execution.
============================ */
function unifiedSelectPackage(btn) {
  ...
  // ── STEP 1: Reset ALL other cards before doing anything else ──
  const card = btn.closest(".package-card");
  resetAllPackageCards(card);

  // ── STEP 2: Save new package to localStorage ──
  ...
  // ── STEP 3: Show Selected state on THIS card ──
  if (card) {
  ...
  // ── STEP 4: Run update/load functions safely ──
  // Each is wrapped individually so a crash in one never
  ...
  // ── STEP 5: Re-apply mutual exclusion AFTER all load* functions ──
  // Safety net: if ANY load* re-rendered card states from
  // localStorage, this final call corrects the DOM.
  resetAllPackageCards(card);
  if (card) {
    const selectedBtn = card.querySelector(".btn-selected");
    const deselectBtn = card.querySelector(".btn-deselect");
...
/* ============================
   ...
   All click handlers registered after DOM is ready.
============================ */
setTimeout(() => {
  ...
  document.querySelectorAll(".select-package-btn").forEach(btn => {
    btn.addEventListener("click", () => {
      // Check if this is the corporate "Request a Quote" button
      const card = btn.closest(".package-card");
